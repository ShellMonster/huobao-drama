...
**2026-01-23 (6项 | +1,624/-201行)**
- 风格管理后台（创建/编辑/删除/批量导入JSON）
- 复用尾帧优化（图片列表显示复用来源）
- 视频时间线增强（全局转场/重叠裁剪）

**2026-01-22 (14项 | +4,297/-1,228行)**
- 项目风格选择（22种内置风格/网格预览）
- 帧提示词系统（首/关键/尾帧/分镜板/动作序列）
- SSE实时推送（异步任务生成/状态跟踪）
- 场景CRUD/SQLite连接池优化/智能轮询

**2026-01-21 (28项 | +6,412/-1,134行)**
- 视频渠道（即梦/可灵/MiniMax/SDK优化）
- 缓存系统（服务端/列表/AI配置/Data URI）
- 国际化完善（英文翻译/角色类型/通用项）
- UX优化（玻璃态加载/Loading组件/导航逻辑）

**2026-01-20 (6项 | +735/-87行)**
- Docker镜像源（国内加速/配置说明）
- 缓存优化（图片视频/跨标签同步）
- Prompt生成（帧提示词/长度限制）
