...
// BatchTransferImagesToMinio is kept for scheduler compatibility. No-op when MinIO is disabled.
func (s *ResourceTransferService) BatchTransferImagesToMinio(dramaID string, limit int) (int, error) {
	return 0, nil
}

// BatchTransferVideosToMinio is kept for scheduler compatibility. No-op when MinIO is disabled.
